...
// Create build archive with advanced compression options
async function createBuildArchive() {
  ...
  // Get optimal compression format
  const format = compression.getOptimalCompressionFormat(projectType);
  
  // Determine output filename based on format
  const outputFile = `build.${format}`;
  ...
  // Create enhanced compression options
  ...
  // Create the compressed archive with the optimal format and options
  return await compression.createCompressedArchive(compressionOptions);
}
...
async function deploy() {
  ...
  try {
    ...
    // Check project size and estimate compression size
    try {
      const sizeInfo = await compression.estimateCompressedSize('.', 
        compression.getOptimalCompressionFormat(projectType));
      
      if (sizeInfo.estimatedSize > 100 * 1024 * 1024) { // 100 MB
        spinner.warn(chalk.yellow(`Large project detected: ${sizeInfo.originalFormatted} (${sizeInfo.fileCount} files)`));
        spinner.warn(chalk.yellow(`Estimated deployment size: ${sizeInfo.estimatedFormatted} (${sizeInfo.compressionRatio} compression)`));
        
        const { shouldContinue } = await inquirer.prompt([
          {
            type: 'confirm',
            name: 'shouldContinue',
            message: 'The deployment package will be very large. Continue?',
            default: true
          }
        ]);
      ...
      // Continue even if size estimation fails
      console.log(chalk.yellow(`Could not estimate project size: ${error.message}`));
    }
    ...
    // Display compressed size info
    const stats = fs.statSync(buildFile);
    spinner.succeed(chalk.green(`Deployment archive created (${compression.formatSize(stats.size)})`));
    spinner.start('Uploading deployment...');
